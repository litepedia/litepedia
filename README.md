...
```bash
litepedia$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
litepedia$ sam local start-api
litepedia$ curl http://localhost:3000/
```
...
```bash
litepedia$ sam logs -n HelloWorldFunction --stack-name litepedia --tail
```
...
```bash
sam delete --stack-name litepedia
```
